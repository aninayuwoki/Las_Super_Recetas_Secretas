...
document.addEventListener('DOMContentLoaded', function() {
    if (document.getElementById('recipe-container')) {
        ...
        if (recipe) {
            ...
            document.getElementById('recipe-container').innerHTML = `<div class="p-10 text-center"><h1 class="font-title text-5xl text-red-600">Error</h1><p class="mt-4 text-xl text-gray-700">Receta no encontrada.</p><a href="index.html" class="mt-8 inline-block bg-amber-600 text-white px-6 py-3 rounded-full font-semibold">Volver al inicio</a></div>`;
        }
    }
});
...
function renderRecipe(recipe) {
    ...
    document.getElementById('recipe-info-cards').innerHTML = recipe.infoCards.map(card => `<div class="bg-white/20 backdrop-blur-sm rounded-full px-4 py-2 text-white flex items-center animate-fadeInUp"><i class="fas ${card.icon} mr-2"></i>${card.stars ? `<div class="mr-2">${[...Array(3)].map((_, i) => `<i class="fas fa-star ${card.stars[i] ? '' : 'text-gray-400'}"></i>`).join('')}</div>` : ''}<span>${card.text}</span></div>`).join('');
    document.getElementById('love-message-container').innerHTML = `<h3 class="font-heading text-2xl mb-3"><i class="fas ${recipe.loveMessage.icon} mr-2 animate-heartbeat"></i> ${recipe.loveMessage.title}</h3><p class="text-lg leading-relaxed">${recipe.loveMessage.text}</p>`;
    ...
    document.getElementById('ingredient-list').innerHTML = recipe.ingredients.items.map(item => `<li class="flex items-center cursor-pointer group"><label class="flex items-center w-full"><input type="checkbox" class="hidden peer ingredient-checkbox"><span class="w-10 h-10 mr-4 border-2 border-gray-300 rounded-xl flex items-center justify-center checkmark group-hover:border-amber-500"><i class="fas fa-check text-white transform scale-0 opacity-0 transition-transform duration-200"></i></span><div class="ingredient-visual bg-gradient-to-br from-amber-100 to-amber-200 group-hover:scale-110">${item.visual}</div><div class="item-text transition-opacity duration-300 flex-grow"><div><strong class="text-xl">${item.amount}</strong> <span class="font-semibold">${item.name}</span><div class="text-sm text-gray-500 mt-1">${item.description}</div></div></div></label></li>`).join('');
    ...
    document.getElementById('instruction-list').innerHTML = recipe.instructions.steps.map((step, i) => `<li class="flex items-start"><label class="flex items-start cursor-pointer group w-full"><input type="checkbox" class="hidden peer" data-step="${i + 1}"><div class="step-container flex-shrink-0 relative"><div class="step-circle w-16 h-16 text-white rounded-full flex items-center justify-center font-bold text-2xl shadow-lg transform transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6">${i + 1}</div></div><div class="flex-grow ml-6"><div class="item-text transition-opacity duration-300"><div class="flex items-center mb-3"><h3 class="font-semibold text-xl mr-3">${step.title}</h3><div class="flex space-x-2">${step.tags.map(tag => `<span class="instruction-tag px-3 py-1 rounded-full text-sm font-semibold">${tag}</span>`).join('')}</div></div><p class="leading-relaxed">${step.text}</p>${step.tip ? `<div class="mt-3 flex items-center text-sm text-gray-500"><i class="fas fa-lightbulb text-yellow-500 mr-2"></i><span><strong>Tip:</strong> ${step.tip}</span></div>` : ''}${step.important ? `<div class="mt-3 p-3 bg-red-50 border-l-4 border-red-400 rounded"><div class="flex items-center text-sm text-red-700"><i class="fas fa-exclamation-triangle mr-2"></i><span><strong>Importante:</strong> ${step.important}</span></div></div>` : ''}${step.action && step.action.type === 'video' ? `<button id="moisesDemoBtn" class="mt-3 bg-blue-500 text-white px-4 py-2 rounded-lg">${step.action.text}</button>` : ''}${step.toppings ? `<div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">${step.toppings.map(t => `<div class="topping-option bg-gray-100 p-3 rounded-lg text-center cursor-pointer hover:scale-105 transition-transform duration-300" data-topping="${t.name}"><div class="text-2xl mb-1">${t.visual}</div><div class="text-xs font-semibold">${t.name}</div></div>`).join('')}</div><div id="selectedToppings" class="mt-3 hidden"><div class="bg-green-50 border-l-4 border-green-400 p-3 rounded"><div class="flex items-center text-sm text-green-700"><i class="fas fa-check-circle mr-2"></i><span>Toppings seleccionados: <strong id="toppingsList"></strong></span></div></div></div>` : ''}</div></div></label></li>`).join('');
...
function createWelcomeParticles() { /* ... particle generation logic ... */ }

function setupStepTracking() {
    document.body.addEventListener('change', e => {
        if (e.target.matches('input[data-step]')) handleStepChange(e.target);
        if (e.target.matches('.ingredient-checkbox')) updateProgress();
    });
}
function handleStepChange(checkbox) { /* ... logic ... */ }
function updateStepCounter() { /* ... logic ... */ }
function updateProgress() { /* ... logic ... */ }
function checkRecipeCompletion() { /* ... logic ... */ }
...
function startTimer() { /* ... logic ... */ }
function pauseTimer() { /* ... logic ... */ }
function resetTimer() { /* ... logic ... */ }
function updateTimerDisplay() { /* ... logic ... */ }
...
function setupBakingMode() {
    ...
    if (btn) btn.addEventListener('click', () => toggleBakingMode(btn));
}
...
function highlightCurrentStep() { /* ... logic ... */ }
function stopGuidedBaking() { /* ... logic ... */ }
function moveToNextStep(step) { /* ... logic ... */ }

function setupRatingSystem() {
    const container = document.getElementById('recipeRating');
    if(!container) return;
    container.addEventListener('click', e => { if (e.target.matches('i')) setRating(parseInt(e.target.dataset.rating)); });
    container.addEventListener('mouseover', e => { if (e.target.matches('i')) highlightStars(parseInt(e.target.dataset.rating)); });
    container.addEventListener('mouseleave', () => highlightStars(parseInt(container.dataset.currentRating || 0)));
}
function highlightStars(count) { /* ... logic ... */ }
function setRating(rating) { /* ... logic ... */ }

function setupModalButtons() {
    document.body.addEventListener('click', e => {
        if (e.target.closest('#shoppingListBtn')) {
            console.log("Shopping list button clicked");
            openModal('shoppingModal');
        }
        if (e.target.closest('#moisesDemoBtn')) {
            console.log("Moises demo button clicked");
            openModal('moisesModal');
        }
...
function setupGeneralEventListeners() {
    ...
    if(shareBtn) shareBtn.addEventListener('click', () => {
        ...
        if (navigator.share) {
            navigator.share({ title: document.title, text: text, url: window.location.href }).catch(err => console.error("Share failed:", err));
        } else {
...
function selectTopping(element, name) { /* ... logic ... */ }
function showNotification(message, type = 'info') { /* ... logic ... */ }
